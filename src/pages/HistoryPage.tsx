...
export default function HistoryPage() {
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <span className="flex items-center gap-1 text-green-400 text-xs font-medium"><CheckCircle size={12} /> Terminée</span>;
      case 'failed':
        return <span className="flex items-center gap-1 text-red-400 text-xs font-medium"><XCircle size={12} /> Échouée</span>;
      case 'sending':
        return <span className="flex items-center gap-1 text-yellow-400 text-xs font-medium"><Clock size={12} /> En cours</span>;
      default:
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-3 text-gray-400 py-12 justify-center">
        <History size={20} className="animate-pulse" /> Chargement...
      </div>
    );
  }
  ...
  if (campaigns.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400">Aucune campagne envoyée pour l'instant.</p>
      </div>
    );
  }
        ...
        {campaigns.map(campaign => (
          <div key={campaign.id} className="bg-gray-800 rounded-lg overflow-hidden">
            ...
            <button
              ...
              className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-750 transition text-left"
            >
